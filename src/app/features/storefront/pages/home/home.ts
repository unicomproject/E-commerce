...
@Component({
  ...
  template: `
    <div class="flex min-h-screen w-full max-w-[100vw] flex-col overflow-x-hidden bg-page-bg lg:min-h-[calc(100vh-3.5rem)]">

      <ng-container *ngIf="!isLoading(); else loadingState">
        <!-- Shared horizontal padding: collection bar + banner share same vertical edges -->
        <div class="flex w-full max-w-[1600px] mx-auto min-w-0 flex-1 flex-col px-4 pt-2 lg:gap-6 lg:px-6 lg:px-8 lg:pt-4 lg:pb-8">

          <app-fulfillment-selector class="shrink-0"></app-fulfillment-selector>

          <app-hero-carousel class="block w-full min-w-0 shrink-0" [banners]="heroBanners()"></app-hero-carousel>

          <div class="flex w-full min-w-0 flex-col gap-4 xl:grid xl:grid-cols-2 xl:items-start xl:gap-6">
            <app-category-strip class="block w-full min-w-0" [categories]="categories()"></app-category-strip>
            <app-promo-banners class="block w-full min-w-0" [banners]="promoBanners()"></app-promo-banners>
          </div>
