...
@Component({
  ...
  template: `
    <div class="mb-5 w-full min-w-0 lg:mb-0" *ngIf="categories().length > 0">
      <div class="mb-4 mt-2 flex items-center justify-between gap-3 lg:mt-0">
        <h3 class="min-w-0 text-[18px] font-bold tracking-tight text-brand-black lg:text-[24px] lg:leading-8">Shop by Category</h3>
        <a routerLink="/categories" class="shrink-0 text-[14px] font-bold text-brand-orange hover:underline">
      ...
      <div class="flex gap-4 overflow-x-auto hide-scrollbar pb-2 pt-2 lg:gap-6 lg:pb-0 lg:pt-0">
        <a
          *ngFor="let cat of categories().slice(0, 6); let i = index"
          [routerLink]="['/search']"
          [queryParams]="{ category: cat.slug }"
          [ngClass]="i >= 5 ? 'hidden md:flex' : 'flex'"
          class="group w-[72px] flex-shrink-0 flex-col items-center md:w-[100px] lg:w-[96px]"
        >
          <div class="relative flex h-[64px] w-[64px] items-center justify-center overflow-hidden rounded-full bg-brand-light-grey transition-transform duration-300 group-hover:scale-105 md:h-[90px] md:w-[90px] lg:h-[96px] lg:w-[96px]">
            <img
              *ngIf="cat.imageUrl"
              [src]="cat.imageUrl"
              [alt]="cat.name"
          ...
          <span class="mt-2 text-center text-[12px] font-medium leading-tight text-brand-black md:text-[14px] lg:text-[12px] lg:leading-4">
            {{ cat.name }}
          </span>
        </a>
      </div>
