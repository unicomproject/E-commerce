...
import { Observable, combineLatest, of, throwError, switchMap, catchError, map, tap } from 'rxjs';
    ...
    this.route.paramMap
          ...
          return this.storefrontData.getCategoryBySlug(slug).pipe(
            switchMap((category) => {
              ...
              return this.storefrontData
                ...
                .pipe(map((children) => ({ category, children })));
            }),
          );
        }),
        catchError((err) => {
      ...
      .subscribe((data) => {
        if (data) {
          ...
          this.totalItems = this.subcategories.reduce(
