...
export class Search implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    ).pipe(takeUntilDestroyed(this.destroyRef)).subscribe(query => {
    ...
    this.route.queryParams.pipe(takeUntilDestroyed(this.destroyRef)).subscribe(params => {
      ...
      if (this.categorySlug() && !this.categoryId()) {
        ...
        this.dataService.getCategoryBySlug(this.categorySlug()).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
          next: (cat) => {
            if (cat && cat.id) {
  ...
  performSearch() {
    ...
    this.dataService.searchProducts(request).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: (res) => {
        setTimeout(() => {
