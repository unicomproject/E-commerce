...
@Component({
  ...
  template: `
    <div
      ...
      [class.border-neutral-100]="!isLast()"
    >
      <!-- Image -->
      <div
        class="h-16 w-16 md:h-20 md:w-20 rounded-lg bg-neutral-50 border border-neutral-100 flex-shrink-0 flex items-center justify-center overflow-hidden"
      >
        @if (item().imageUrl) {
          <img
            [src]="item().imageUrl"
      ...
      <!-- Info -->
      <div class="flex-grow">
        <h4 class="font-bold text-brand-navy text-sm md:text-base leading-tight">
          {{ item().productName }}
        </h4>
        @if (item().variantName) {
