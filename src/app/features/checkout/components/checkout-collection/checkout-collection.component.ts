...
@Component({
  ...
  viewProviders: [
    provideIcons({ lucideInfo, lucideMapPin, lucideCalendar, lucideClock, lucideChevronRight }),
  ],
  template: `
    <div class="animate-in fade-in duration-300">
      ...
      <div class="mb-6">
        ...
        <p class="text-gray-500 text-sm">
          Choose your collection outlet, date and time for pickup.
        </p>
      </div>
      ...
      <div class="mb-8">
          ...
          <div class="grid gap-3">
            @for (store of checkoutService.availableStores(); track store.id) {
              ...
              >
                ...
                <div
                  class="w-20 h-20 bg-gray-200 rounded-lg overflow-hidden shrink-0 flex items-center justify-center text-gray-400"
                >
                ...
                <div class="flex-1">
                  ...
                  <div class="flex items-center text-xs">
                    <div
                      ...
                      [ngClass]="store.isOpen ? 'bg-green-500' : 'bg-red-500'"
                    ></div>
                    ...
                    @if (store.isOpen) {
                      ...
                      <span class="text-gray-500"
                        >Collection available in as little as
                        {{ store.preparationLeadMinutes }} mins</span
                      >
                    }
                  </div>
                </div>
        ...
        <!-- Info Banner -->
        <div
          class="bg-orange-50 border border-orange-100 rounded-xl p-4 flex gap-3 items-start mt-4"
        >
          ...
          <div>
            ...
            <p class="text-gray-600 text-sm mt-0.5">
              Changing the collection outlet may affect item availability. Some items in your cart
              may not be available at the new outlet.
            </p>
          </div>
        </div>
      </div>
            ...
            @for (dateObj of checkoutService.collectionOptions()?.dates; track dateObj.date) {
              ...
              >
                ...
                <span
                  class="text-xs"
                  [ngClass]="
                    selectedDate()?.date === dateObj.date ? 'text-brand-orange' : 'text-gray-500'
                  "
                  >{{ formatDate(dateObj.date) }}</span
                >
              </div>
            }
          </div>
        ...
        @if (selectedDate()) {
          <div class="mb-8">
            ...
            <p class="text-sm text-gray-500 mb-3">
              For {{ checkoutService.collectionOptions()?.outletName }} •
                ...
                <div
                  ...
                  [ngClass]="{
                    'border-brand-orange bg-orange-50 text-brand-orange font-bold':
                      selectedTime()?.startAt === window.startAt,
                    'border-gray-200 hover:border-gray-300 text-gray-900 font-medium bg-white':
                      selectedTime()?.startAt !== window.startAt,
                  }"
      ...
      <div class="border border-gray-200 bg-white rounded-xl p-4 flex gap-4 items-start mb-4">
        ...
        <div>
          ...
          <p class="text-gray-600 text-sm mt-0.5">
            Please arrive within 15 minutes of your selected time and bring your order confirmation.
          </p>
        </div>
      </div>
      ...
      <!-- Actions -->
      <div
        class="sticky bottom-0 bg-white pt-4 pb-4 md:pb-6 z-10 -mx-6 px-6 -mb-6 space-y-3 border-t border-gray-100 mt-6"
      >
        <button
          type="button"
  ...
  onContinue() {
    ...
    this.checkoutService
      ...
      .subscribe({
        next: (res) => {
          if (!res.success) {
            this.toastService.error(res.message || 'Some items are not available at this outlet.');
          }
        },
        error: (err: any) => {
          this.toastService.error('Failed to update collection outlet. Please try again.');
        }
      });
  }
