...
import { lucideUser, lucideMail, lucidePhone, lucideStore, lucideMapPin, lucideCalendar, lucideEdit2, lucideLock, lucideCheckCircle2 } from '@ng-icons/lucide';
import { CheckoutService } from '../../../../core/services/checkout.service';
import { StorefrontDataService } from '../../../storefront/services/storefront-data.service';
import { TenantCurrencyPipe } from '../../../../shared/pipes/tenant-currency.pipe';
import { CollectionTimeModalComponent } from '../../../storefront/components/collection-time-modal/collection-time-modal';

@Component({
  selector: 'app-checkout-review',
  standalone: true,
  imports: [CommonModule, NgIconComponent, TenantCurrencyPipe, CollectionTimeModalComponent],
  viewProviders: [provideIcons({ lucideUser, lucideMail, lucidePhone, lucideStore, lucideMapPin, lucideCalendar, lucideEdit2, lucideLock, lucideCheckCircle2 })],
  template: `
    <div class="animate-in fade-in duration-300">
      ...
      @if (checkoutService.checkoutSession() !== null) {
        ...
        <!-- Customer Details -->
        <div class="border border-gray-100 bg-white rounded-xl shadow-[0_2px_10px_-4px_rgba(0,0,0,0.1)] p-4 mb-4 relative group">
          ...
          <div class="space-y-3">
            <div class="flex items-center gap-3 text-gray-700">
              ...
              <span class="text-sm">{{ checkoutService.checkoutSession()!.pickupContactName }}</span>
            </div>
            <div class="w-full border-t border-gray-100 pl-8"></div>
            <div class="flex items-center gap-3 text-gray-700">
              <ng-icon name="lucideMail" size="18" class="text-gray-400 shrink-0"></ng-icon>
              <span class="text-sm">{{ checkoutService.checkoutSession()!.pickupContactEmail }}</span>
            </div>
            <div class="w-full border-t border-gray-100 pl-8"></div>
            <div class="flex items-center gap-3 text-gray-700">
              <ng-icon name="lucidePhone" size="18" class="text-gray-400 shrink-0"></ng-icon>
              <span class="text-sm">{{ checkoutService.checkoutSession()!.pickupContactPhone }}</span>
            </div>
          </div>
        </div>
          ...
          <div class="space-y-3">
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-3 text-gray-700">
                ...
                <span class="text-sm">{{ checkoutService.checkoutSession()!.selectedOutletName }}</span>
              </div>
        ...
        <!-- Your Items -->
        <div class="mb-6">
          <h3 class="text-[17px] font-bold text-gray-900 mb-3">Your Items ({{ checkoutService.checkoutSession()!.totalQuantity }})</h3>
          
          <div class="border border-gray-100 bg-white rounded-xl shadow-[0_2px_10px_-4px_rgba(0,0,0,0.1)] p-4">
            ...
            <div class="space-y-4 max-h-[300px] overflow-y-auto pr-2 hide-scrollbar mb-4">
              @for (item of checkoutService.checkoutSession()!.items; track item.id) {
                <div class="flex gap-4 items-start">
                  <!-- Product Image -->
                  <div class="w-[84px] h-[64px] bg-gray-100 rounded-lg shrink-0 flex items-center justify-center overflow-hidden border border-gray-100">
                    @if (item.imageUrl) {
                  ...
                  <div class="flex-1 min-w-0 pt-1">
                    <h4 class="font-bold text-[15px] text-gray-900 leading-tight truncate">{{ item.productName }}</h4>
                    <p class="text-gray-400 text-xs mt-1 uppercase">{{ item.sku || 'N/A' }}</p>
                    <p class="text-gray-500 text-sm mt-1">Qty: {{ item.quantity }}</p>
                  </div>
                  
                  <div class="shrink-0 pt-1">
                    <span class="font-bold text-[15px] text-gray-900">{{ item.lineTotal | tenantCurrency:'symbol':'1.2-2' }}</span>
                  </div>
                </div>
              }
            </div>
            ...
            <div class="border-t border-gray-100 pt-4 space-y-2">
              ...
              <div class="flex justify-between text-gray-500 text-[13px]">
                <span>VAT {{ checkoutService.checkoutSession()!.isTaxInclusive ? '(included)' : '(12%)' }}</span>
            ...
            <div class="border-t border-gray-100 mt-3 pt-3 flex justify-between items-center">
              ...
              <span class="font-bold text-lg text-gray-900">{{ checkoutService.checkoutSession()!.grandTotal | tenantCurrency:'symbol':'1.2-2' }}</span>
            </div>
        ...
        <div class="flex items-start gap-2 text-gray-500 text-xs mb-6 px-1">
          ...
          <span class="leading-tight mt-1">Payment is securely processed in-store at collection.</span>
        </div>

        <!-- Actions -->
        <div class="sticky bottom-0 bg-white pt-4 pb-4 md:pb-6 z-10 -mx-6 px-6 -mb-6 flex flex-col items-center border-t border-gray-100">
          <button 
            type="button" 
          ...
          >
            <span *ngIf="checkoutService.isLoading()" class="mr-2 border-2 border-white border-t-transparent rounded-full w-4 h-4 animate-spin"></span>
            <ng-icon *ngIf="!checkoutService.isLoading()" name="lucideLock" size="18" class="mr-2"></ng-icon>
            Place Collection Order • {{ checkoutService.checkoutSession()!.grandTotal | tenantCurrency:'symbol':'1.0-0' }}
          </button>
        </div>
