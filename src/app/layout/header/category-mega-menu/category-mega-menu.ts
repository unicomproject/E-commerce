...
@Component({
  ...
  standalone: true,
  imports: [RouterModule, NgIconComponent, LoadingSpinnerComponent],
  viewProviders: [provideIcons({ lucideChevronRight })],
  template: `
    <div
      class="absolute top-full left-0 w-[850px] bg-white border border-gray-200 shadow-xl rounded-b-xl z-50 flex overflow-hidden min-h-[400px] max-h-[500px]"
    >
      <!-- Left Sidebar: Root Categories -->
      <div class="w-1/3 bg-gray-50 border-r border-gray-200 overflow-y-auto py-2">
        <ul class="flex flex-col">
          @for (root of rootCategories(); track root) {
            <li
      ...
      <div class="w-2/3 bg-white flex flex-col h-full overflow-hidden relative">
        ...
        <div class="px-6 pt-6 pb-4 shrink-0 bg-white z-10">
          @if (activeRoot()) {
            <h3 class="text-lg font-bold text-gray-900">{{ activeRoot()?.name }}</h3>
          }
        </div>
        ...
        <div class="px-6 pb-6 overflow-y-auto flex-1">
          ...
          @if (!isLoadingChildren() && activeChildren().length > 0) {
            <div class="grid grid-cols-3 gap-y-8 gap-x-4">
              @for (child of activeChildren(); track child) {
                <a
                  [routerLink]="['/search']"
                  [queryParams]="{ category: child.slug }"
                  class="flex flex-col items-center group cursor-pointer text-center"
                >
                  <div
                    class="w-24 h-24 rounded-2xl overflow-hidden bg-gray-50 mb-3 shadow-sm group-hover:shadow-md transition-shadow ring-1 ring-black/5 group-hover:ring-brand-orange/30 flex items-center justify-center"
                  >
                    <img
                      [src]="
                        child.imageUrl ||
                        'https://images.unsplash.com/photo-1555529733-0e670560f4e1?w=200&h=200&fit=crop'
                      "
                      [alt]="child.name"
                  ...
                  <span
                    class="text-xs font-medium text-gray-700 group-hover:text-brand-orange transition-colors line-clamp-2 leading-tight"
                    >{{ child.name }}</span
                  >
                </a>
              }
          ...
          @if (
            ...
            <div class="flex flex-col h-full">
              <div class="grid grid-cols-3 gap-y-8 gap-x-4 mb-4">
                @for (product of activeProducts(); track product) {
                  <a
                    [routerLink]="['/product', product.slug]"
                    class="flex flex-col items-center group cursor-pointer text-center"
                  >
                    <div
                      class="w-24 h-24 rounded-2xl overflow-hidden bg-white mb-3 shadow-sm group-hover:shadow-md transition-shadow ring-1 ring-black/5 group-hover:ring-brand-orange/30 flex items-center justify-center"
                    >
                      <img
                        [src]="
                          product.imageUrl ||
          ...
          @if (
            ...
            <div class="flex flex-col items-center justify-center h-32 text-gray-400">
              ...
              <a
                [routerLink]="['/search']"
                [queryParams]="{ category: activeRoot()?.slug }"
                class="mt-2 text-sm text-brand-orange hover:underline"
                >View all in {{ activeRoot()?.name }}</a
              >
            </div>
          }
        </div>
        ...
        @if (
          ...
          <div
            class="p-4 border-t border-gray-100 bg-white shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.02)] shrink-0 z-10 flex justify-center"
          >
            <a
              [routerLink]="['/search']"
              [queryParams]="{ category: activeRoot()?.slug }"
              class="inline-flex items-center gap-2 text-sm font-semibold text-brand-orange hover:text-brand-orange-dark hover:underline transition-colors"
            >
...
export class CategoryMegaMenu implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.hoverSubscription = this.hoverSubject.pipe(debounceTime(200)).subscribe((root) => {
