...
/** Find a CountryCode entry by ISO code (e.g. 'IN') */
export function findByCode(code: string): CountryCode | undefined {
  return COUNTRY_CODES.find(c => c.code === code.toUpperCase());
}

/** Find a CountryCode entry by dial prefix string found in a full E.164 phone */
export function findByPhone(phone: string): CountryCode | undefined {
  if (!phone) return undefined;
  const normalized = phone.startsWith('+') ? phone : '+' + phone;
  // Longest match wins to avoid e.g. +1 matching before +1-xxx
  return COUNTRY_CODES
    .filter(c => normalized.startsWith(c.dial))
    .sort((a, b) => b.dial.length - a.dial.length)[0];
}

/** Strip the dial code from an E.164 number, returning just the subscriber number */
export function stripDialCode(phone: string, dialCode: string): string {
  if (!phone) return '';
  const normalized = phone.startsWith('+') ? phone : '+' + phone;
  return normalized.startsWith(dialCode) ? normalized.slice(dialCode.length) : normalized;
}
